...
function freshBot(symbol,targetPct){return {id:botId(symbol,targetPct),symbol,pair:ASSETS[symbol].pair,label:ASSETS[symbol].label,baseCapital:1000,targetPct,feePct:0,currentPrice:null,entryPrice:null,tokensHeld:0,bankedProfit:0,harvestCount:0,running:true,lastUpdated:null,lastError:null,history:[]};}
...
function loadState(){try{if(fs.existsSync(STATE_FILE)){const saved=JSON.parse(fs.readFileSync(STATE_FILE,'utf8'));if(saved.bots){const bots={};for(const s of Object.keys(ASSETS))for(const t of TARGETS){const f=freshBot(s,t),p=saved.bots[f.id];bots[f.id]=p?{...f,...p,id:f.id,symbol:s,pair:f.pair,label:f.label,targetPct:t,baseCapital:1000}:f;}return {bots};}}}catch(e){console.error('State load failed:',e.message);}return makeDefaults();}
let state=loadState();
function saveState(){try{fs.writeFileSync(STATE_FILE,JSON.stringify(state,null,2));}catch(e){console.error('State save failed:',e.message);}}
