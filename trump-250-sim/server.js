...
function fresh(){return{
  mode:'SIMULATOR',symbol:SYMBOL,capitalUsdt:CAPITAL,targetPct:TARGET_PCT,feePct:FEE_PCT,minOrderUsdt:MIN_ORDER_USDT,
  startedAt:null,lastUpdated:null,lastPrice:null,anchorPrice:null,tokensHeld:0,
  harvestCount:0,grossHarvestedUsdt:0,feesPaidUsdt:0,netHarvestedUsdt:0,
  lastHarvest:null,history:[],lastError:null
};}
...
async function fetchMarket(){const [t,inst]=await Promise.all([
  publicGet('/v5/market/tickers',{category:'spot',symbol:SYMBOL}),
  publicGet('/v5/market/instruments-info',{category:'spot',symbol:SYMBOL})
]);
  const price=Number(t.list?.[0]?.lastPrice);if(!Number.isFinite(price)||price<=0)throw new Error('TRUMP price unavailable');
  const minOrderAmt=Number(inst.list?.[0]?.lotSizeFilter?.minOrderAmt||MIN_ORDER_USDT);
  return{price,minOrderAmt};
}
...
function evaluate(price,minOrderAmt){state.lastPrice=price;state.minOrderUsdt=Math.max(MIN_ORDER_USDT,minOrderAmt||0);state.lastUpdated=new Date().toISOString();if(!state.anchorPrice){initialize(price);return;}
  const trigger=state.anchorPrice*(1+TARGET_PCT/100);
  const value=positionValue();
  const grossExcess=Math.max(0,value-CAPITAL);
  if(price+Number.EPSILON>=trigger && grossExcess+1e-12>=state.minOrderUsdt){
    const fee=grossExcess*(FEE_PCT/100);
    const net=grossExcess-fee;
    const soldTokens=grossExcess/price;
    state.tokensHeld=Math.max(0,state.tokensHeld-soldTokens);
    state.harvestCount++;
    state.grossHarvestedUsdt+=grossExcess;
    state.feesPaidUsdt+=fee;
    state.netHarvestedUsdt+=net;
    state.lastHarvest={number:state.harvestCount,time:state.lastUpdated,price:round(price,8),gross:round(grossExcess,6),fee:round(fee,6),net:round(net,6)};
    state.history.push(state.lastHarvest);
